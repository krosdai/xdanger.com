...
| 合作模式 | 你负责 | EDA 谁买 |
|---|---|---|
| 全包（turnkey） | 只给规格 | 设计公司全包，成本折进 NRE |
| 半包（RTL-in） | 前端 RTL（寄存器传输级）+ 验证 | 前端自购，后端对方出 |
| 完全自研 | 全流程 | 自购整套 |

外包越深越不必自购，但 EDA 成本始终折进你付的 NRE（一次性工程费）。超大厂多走半包，仍握前端许可证；只有设计量足够大，自养团队 + 买整套才划算——这就是 Apple、Nvidia 全自研的经济理由。
...
<HbmConsignmentFlow caption="HBM 的「自带料」流向：产品方直采内存 + 预付锁产能，连同台积电流片的 logic die 一起交台积电做 CoWoS——代工厂只封装、不代采。" />
...
<FoundryLockInStack caption="以晶圆厂为中心的认证金字塔：每一层都要 foundry 或节点级认证，换厂须整链重认证——这就是 lock-in 的来源。" />
...
3. **小芯片化 + UCIe。** 跨厂、跨节点把 die（裸片）拼起来，催生「已知良品 die（KGD）」与 die-to-die 接口的新规矩，长期松动单一晶圆厂的锁定。
